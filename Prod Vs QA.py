...
for prod_file,qa_file,dp_id in zip(df['prod_files'], df['qa_files'], df.index):
    ...
    df_prod['Valuation Error']=df_prod['Valuation Error'].astype(str) 
    tp_prod=len(df_prod[df_prod['Valuation Status']=='OK'])
    tnp_prod=len(df_prod[df_prod['Valuation Status']!='OK'])
    df_qa['Valuation Error']=df_qa['Valuation Error'].astype(str)
    
    tp_qa=len(df_qa[df_qa['Valuation Status']=='OK'])
    tnp_qa=len(df_qa[df_qa['Valuation Status']!='OK'])
    
    df_merge_left=df_prod.merge(df_qa,on='Trade ID', how='left', indicator=True, suffixes=('','_qa'))
    
    df_only_prod=df_merge_left[df_merge_left['_merge']=='left_only'] 
    df_only_prod=df_only_prod[df_prod.columns]
    
    tp_only_prod=len(df_only_prod[df_only_prod['Valuation Status']=='OK'])
    tnp_only_prod=len(df_only_prod[df_only_prod['Valuation Status']!='OK'])
    
    df_merge_right=df_prod.merge(df_qa, on='Trade ID', how='right', indicator=True, suffixes=('_prod',''))
    df_only_qa=df_merge_right[df_merge_right['_merge']=='right_only']
    df_only_qa=df_only_qa[df_qa.columns]
    
    tp_only_qa=len(df_only_qa[df_only_qa['Valuation Status']=='OK'])
    tnp_only_qa=len(df_only_qa[df_only_qa['Valuation Status']!='OK'])
    
    df_both=df_merge_left[df_merge_left['_merge']=='both']
    
    df_prod.set_index('Trade ID', inplace=True) 
    df_qa.set_index('Trade ID', inplace=True)
    
    df_only_prod.set_index('Trade ID', inplace=True)
    df_only_qa.set_index('Trade ID', inplace=True)
    ...
    df_comp=df_comp[df_comp['Trade ID'].isin(df_both['Trade ID'])]
    df_comp.set_index('Trade ID', inplace=True) 
    
    df_comp_num.reset_index(inplace=True)
    df_comp_num=df_comp_num[df_comp_num['Trade ID'].isin(df_both['Trade ID'])]
    df_comp_num.set_index('Trade ID', inplace=True)
...
#-status
    ...
    df_prod_c=df_prod_c[df_prod_c['Trade ID'].isin(df_comp_c['Trade ID'])]
    df_qa_c=df_qa_c[df_qa_c['Trade ID'].isin(df_comp_c['Trade ID'])]
    
    df_prod_c.set_index('Trade ID', inplace=True) 
    df_qa_c.set_index('Trade ID', inplace=True)
    
    df_comp_c.set_index('Trade ID', inplace=True)
    ...
    trades_priced.append([tp_prod, tnp_prod, tp_qa, tnp_qa, tp_only_prod, tnp_only_prod, tp_only_qa,tnp_only_qa])
...
for ids, files,p,q,po, qo, tp, s, nm in zip(dp_ids,comp,prod,qa,prod_only,qa_only,trades_priced, status, num_match):
    ...
    df_kpi.append({'Dote': date, 'SNAP': SNAP, 'Curve': Curve, 'Asset':Asset, 'Prod':len(p), 
                   ...
                   'TP_Prod': tp[0], 'TNP_Prod':tp[1], 'TP_QA':tp[2], 'TNP_QA': tp[3],    
                   'TP_Only_Prod': tp[4], 'TNP_Only_Prod': tp[5], 'TP_Only_QA':tp[6], 
                   'TNP_Only_QA': tp[7], 'Status Cat':s[0], 'Not Matched Cat Fields':s[1]})
...
for ids, files,p,q,po, qo, tp, s, nm in zip(dp_ids,comp,prod,qa,prod_only,qa_only,trades_priced, status, num_match):
    ...
    df_pkpi.append({'Date':date, 'SNAP': SNAP, 'Curve': Curve, 'Asset':Asset, 'Prod':len(p), 
                    ...
                    'TP_Prod': tp[0]/len(p), 'TNP_Prod':tp[1]/len(p), 'TP_QA':tp[2]/len(q), 'TNP_QA':tp[3]/len(q), 
                    'TP_Only_Prod':0 if len(po)==0 else tp[4]/len(po), 'TNP_Only_Prod':0 if len(po)==0 else tp[5]/len(po),
                    'TP_Only_QA':0 if len(qo)==0 else tp[6]/len(qo), 'TNP_Only_QA':0 if len(qo)==0 else tp[7]/len(qo), 
